...
public function mapAction2(Event $event,$id,MapRepository $m)
{
    ...
    {
        $med=$this->getDoctrine()->getRepository(Event::class)->findAll();
    ...
    public function addmobevent(Request $request,NormalizerInterface $normalizer,EntityManagerInterface $entityManager)
    {
        ...
        $event->setNbr($request->get('nbr'));
        $rest=substr($request->get('date'), 0, 20);
        $rest1=substr($request->get('date'), 30, 34);
        $res=$rest.$rest1;
        try {
        ...
        $rest=substr($request->get('datef'), 0, 20);
        $rest1=substr($request->get('datef'), 30, 34);
        $res=$rest.$rest1;
        try {
            $date = new \DateTime($res);
        ...
        $event->setBordercolor("#440066");
        $event->setTextcolor("#ffffff");
        $event->setEntId($request->get('iduser'));
        $event->setParId(0);
